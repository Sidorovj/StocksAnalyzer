...
#pragma warning restore 4014
		...
		private async Task FillStockForm(bool getNewInfo = true)
		{
			...
			foreach (var coef in Coefficient.CoefficientList)
			{
				...
				if (m_selectedStock.ListToRatings.ContainsKey(m_selectedList) &&
			...
			foreach (var metric in Coefficient.MetricsList)
			{
				...
				if (m_selectedStock.ListToRatings.ContainsKey(m_selectedList) &&
				    m_selectedStock.ListToRatings[m_selectedList].ContainsKey(metric) && Stock.AllStocksInListAnalyzed)
				{
